...
def _parse_skip_patterns(value: str) -> Optional[list[str]]:
    if not value:
        return None
...
def run(
    ...
    def cleanup_resources():
        """清理资源（仅清理中间变量，模型在函数末尾清理）"""
        try:
            # 只清理GPU缓存，不清理模型（模型在循环外部管理）
    ...
    # 加载 CodeRankEmbed 模型（每个进程只加载一次）
    model = None
    tokenizer = None
    try:
        _emit_log(
            f'[Process {rank}] Loading CodeRankEmbed model on GPU {actual_gpu_id}...',
            use_tqdm,
            log_queue,
        )
    ...
    builder = IndexBuilder(args, use_direct_llamaindex_params=True)
    ...
    while True:
        ...
        try:
            show_block_progress = use_tqdm and args.num_processes == 1
            ...
            _emit_log(
                f"[Process {rank}] Collected {block_count} blocks from {repo_name}",
                use_tqdm,
                log_queue,
            )
            _emit_log(
...
def main():
    ...
    parser.add_argument("--llamaindex_language", type=str, default="python",
                       help="编程语言类型（用于 llamaindex_code）")
    ...
    log_queue = mp.Queue() if use_tqdm and args.num_processes > 1 else None
    ...
    if args.num_processes == 1:
        run(0, repo_queue, args, gpu_ids, len(repos), completed_repos, use_tqdm, log_queue)
    else:
